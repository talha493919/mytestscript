...
// RPC URLs
const RPC_URLS = {
    Ethereum: "https://mainnet.infura.io/v3/903b7ac3925c4e9892a78a8e6926d488",
    BSC: "https://bsc-dataseed.binance.org/",
    Solana: "https://api.mainnet-beta.solana.com"
};

// User-defined words (modify this)
const myWords = [
...
// Function to randomly select 12 words
...
async function getSolanaBalance(address) {
    try {
        ...
        return (balance / 1e9).toFixed(8); // Convert lamports to SOL
    } catch (error) {
        return "0";
    }
}

// Function to save data as a file (since GitHub Pages can't write files)
...
async function checkWallet(mnemonic) {
    ...
    if (totalBalance > 0) {
        console.log("✅ Found a funded wallet!");
        saveToFile(
            "found_wallets.txt",
            `Mnemonic: ${mnemonic}\nETH: ${ethAddress}, BTC: ${btcAddress}, SOL: ${solAddress}, BNB: ${bnbAddress}\nBalances: ${JSON.stringify(balances, null, 2)}\n\n`
        );
        return true;
    } else {
        console.log("⚠️ Empty wallet.");
        saveToFile(
            "empty_wallets.txt",
            `Mnemonic: ${mnemonic}\nETH: ${ethAddress}, BTC: ${btcAddress}, SOL: ${solAddress}, BNB: ${bnbAddress}\n\n`
        );
...
async function startChecking(maxConcurrentChecks = 5) {
    ...
    async function processMnemonic() {
        ...
        if (result) {
            foundWallet = true;
            console.log("✅ Found a funded wallet! Stopping...");
        } else {
            queue.delete(this);
    ...
    // Fill the queue with initial tasks
    for (let i = 0; i < maxConcurrentChecks; i++) {
        queue.add(processMnemonic.call({}));
    }

    // Wait until all tasks complete or a wallet is found
    await Promise.allSettled(queue);

    console.log("✅ Finished checking all possible mnemonics.");
}
